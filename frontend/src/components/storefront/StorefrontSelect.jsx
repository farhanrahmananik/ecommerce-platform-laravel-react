...
function StorefrontSelect({
  ...
  const controlId = id || `storefront-select-${generatedId}`
  ...
  return (
    <div
      className={`storefront-select ${isOpen ? 'is-open' : ''} ${disabled ? 'is-disabled' : ''}`}
      ref={rootRef}
      onBlur={(event) => {
    ...
    >
      <span className="storefront-select__label" id={labelId}>
        {label}
      </span>
      <button
        className="storefront-select__control"
        id={controlId}
        type="button"
        ref={controlRef}
      ...
      {isOpen && (
        <div className="storefront-select__menu" id={menuId} role="listbox">
          {options.map((option, index) => {
            const isSelected = normalizeValue(option.value) === normalizedValue

            return (
              <button
                className={`storefront-select__option ${isSelected ? 'is-selected' : ''}`}
                type="button"
                role="option"
