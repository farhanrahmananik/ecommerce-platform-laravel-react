...
function AdminProductReviewsPage() {
  ...
  const moderate = async (review, nextStatus) => {
    ...
    const result = await Swal.fire({
      ...
      customClass: { popup: 'storefront-alert', input: 'admin-moderation-note', confirmButton: rejecting ? 'swal-danger-button' : 'swal-brand-button', cancelButton: 'swal-soft-button' },
    })
  ...
  return (
    <main className="admin-reviews-page admin-list-page app-page-shell">
      <header className="category-list-heading admin-list-header app-page-header admin-commerce-header admin-reviews-header"><div className="admin-list-title-group"><span className="admin-eyebrow app-page-eyebrow">Trust and quality</span><h1 className="app-page-title">Product reviews</h1><p className="app-page-subtitle">Moderate verified customer feedback before it reaches the storefront.</p></div><button className="admin-refresh-button" type="button" onClick={refresh} disabled={loading} aria-label="Refresh product reviews"><i className={`bi bi-arrow-clockwise ${loading ? 'spin' : ''}`} /></button></header>
      <section className="admin-review-stats">
        {[['Loaded', reviews.length, 'bi-chat-square-text', 'blue'], ['Pending', reviews.filter((r) => r.status === 'pending').length, 'bi-hourglass-split', 'amber'], ['Approved', reviews.filter((r) => r.status === 'approved').length, 'bi-check-circle', 'green'], ['Verified', reviews.filter((r) => r.is_verified_purchase).length, 'bi-patch-check', 'violet']].map(([label, value, icon, tone]) => <article className="app-stat-card admin-review-stat" key={label}><span className={`tone-${tone}`}><i className={`bi ${icon}`} /></span><div><small>{label}</small><strong>{value}</strong></div></article>)}
      </section>
      <section className="category-list-card admin-reviews-card admin-table-card">
        <div className="admin-reviews-toolbar admin-table-toolbar admin-filter-card">
          <form className="category-search-form admin-review-search admin-filter-control" onSubmit={submitSearch}><i className="bi bi-search" /><input type="search" value={searchInput} onChange={(e) => setSearchInput(e.target.value)} placeholder="Search review, product, or customer..." /><button type="submit">Search</button></form>
          <div className="admin-review-filters admin-filter-grid"><div className="admin-select-filter"><AdminSelect id="review-status" name="status" label="Status" value={status} options={statusOptions} onChange={updateFilter(setStatus)} /></div><div className="admin-select-filter"><AdminSelect id="review-rating" name="rating" label="Rating" value={rating} options={ratingOptions} onChange={updateFilter(setRating)} /></div>{hasFilters && <button className="category-clear-button" type="button" onClick={clearFilters}><i className="bi bi-x-circle" /> Clear</button>}</div>
        </div>
