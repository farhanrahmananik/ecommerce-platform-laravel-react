...
function StockManagementPage() {
  ...
  return <main className="admin-stock-page">
    <header className="category-list-heading"><div><span className="admin-eyebrow">Inventory control</span><h1>Stock Management</h1><p>Monitor availability, correct inventory, and audit every stock change.</p></div><button className="admin-refresh-button" type="button" onClick={refresh} disabled={loading}><i className={`bi bi-arrow-clockwise ${loading ? 'spin' : ''}`} /></button></header>
    <section className="stock-summary-grid">{[['Products shown', products.length, 'bi-box-seam', 'blue'], ['Low stock', lowCount, 'bi-exclamation-triangle', 'amber'], ['Out of stock', outCount, 'bi-box2', 'red'], ['Units shown', totalUnits, 'bi-stack', 'green']].map(([label, value, icon, tone]) => <article key={label}><span className={`tone-${tone}`}><i className={`bi ${icon}`} /></span><div><small>{label}</small><strong>{value}</strong></div></article>)}</section>
    <section className="category-list-card stock-list-card"><div className="stock-toolbar"><form className="category-search-form" onSubmit={submitSearch}><i className="bi bi-search" /><input type="search" value={searchInput} onChange={(e) => setSearchInput(e.target.value)} placeholder="Search name, slug, or SKU..." /><button type="submit">Search</button></form><button className={`stock-low-filter ${lowStock ? 'active' : ''}`} type="button" onClick={() => { setLoading(true); setLowStock((v) => !v); setPage(1) }}><i className="bi bi-exclamation-triangle" /> Low stock only</button></div>
      ...
      {loading ? <div className="category-table-loading">{[1,2,3,4].map((n)=><span key={n}/>)}</div> : products.length === 0 ? <div className="category-empty-state"><span className="category-empty-icon"><i className="bi bi-boxes" /></span><h2>No stock products found</h2><p>Try clearing the current search or low-stock filter.</p></div> : <><div className="table-responsive category-table-wrap"><table className="table category-table stock-table align-middle mb-0"><thead><tr><th>Product</th><th>Status</th><th>Stock</th><th>Price</th><th>Updated</th><th className="text-end">Actions</th></tr></thead><tbody>{products.map((product) => { const [tone,label]=stockState(product.stock_quantity); return <tr key={product.id}><td data-label="Product"><div className="stock-product-cell"><span>{product.primary_image?.url ? <img src={product.primary_image.url} alt=""/> : <i className="bi bi-box-seam"/>}</span><div><strong>{product.name}</strong><small>{product.sku || product.slug}</small></div></div></td><td data-label="Status"><span className={`category-status-badge ${product.is_active ? 'is-active':'is-inactive'}`}><span/>{product.is_active?'Active':'Inactive'}</span></td><td data-label="Stock"><span className={`stock-level is-${tone}`}><strong>{product.stock_quantity}</strong><small>{label}</small></span></td><td data-label="Price"><strong>{formatAmount(product.price)}</strong></td><td data-label="Updated"><span className="category-created-date">{formatDate(product.updated_at)}</span></td><td data-label="Actions"><div className="stock-actions"><button type="button" onClick={()=>openAdjust(product)}><i className="bi bi-sliders"/> Adjust Stock</button><button type="button" onClick={()=>openHistory(product)}><i className="bi bi-clock-history"/> Movements</button></div></td></tr>})}</tbody></table></div>{meta?.last_page>1&&<footer className="category-pagination"><span>Showing <strong>{meta.from||0}</strong> to <strong>{meta.to||0}</strong> of <strong>{meta.total}</strong></span><nav><button type="button" disabled={page<=1} onClick={()=>{setLoading(true);setPage(p=>p-1)}}><i className="bi bi-chevron-left"/></button><span>Page {meta.current_page} of {meta.last_page}</span><button type="button" disabled={page>=meta.last_page} onClick={()=>{setLoading(true);setPage(p=>p+1)}}><i className="bi bi-chevron-right"/></button></nav></footer>}</>}
    </section>
    {adjusting && <div className="stock-modal-backdrop"><section className="stock-modal" role="dialog" aria-modal="true"><header><div><span>Inventory adjustment</span><h2>{adjusting.name}</h2></div><button type="button" onClick={()=>setAdjusting(null)}><i className="bi bi-x-lg"/></button></header><form onSubmit={submitAdjustment}><label htmlFor="stock-quantity">New stock quantity</label><input id="stock-quantity" type="number" min="0" step="1" value={adjustForm.quantity} onChange={(e)=>setAdjustForm(f=>({...f,quantity:e.target.value}))}/>{adjustErrors.quantity?.map(m=><span className="category-field-error" key={m}>{m}</span>)}<label htmlFor="stock-reason">Reason <small>Optional</small></label><textarea id="stock-reason" rows="4" maxLength="1000" value={adjustForm.reason} onChange={(e)=>setAdjustForm(f=>({...f,reason:e.target.value}))}/>{adjustErrors.reason?.map(m=><span className="category-field-error" key={m}>{m}</span>)}<footer><button className="btn btn-admin-soft" type="button" onClick={()=>setAdjusting(null)}>Cancel</button><button className="btn btn-admin-primary" type="submit" disabled={saving}>{saving?'Saving...':'Save adjustment'}</button></footer></form></section></div>}
    {historyProduct && <div className="stock-modal-backdrop"><section className="stock-modal stock-history-modal" role="dialog" aria-modal="true"><header><div><span>Movement history</span><h2>{historyProduct.name}</h2></div><button type="button" onClick={()=>setHistoryProduct(null)}><i className="bi bi-x-lg"/></button></header><div className="stock-movement-list">{movementError&&<div className="category-alert category-alert-danger">{movementError}</div>}{movementLoading?<div className="category-table-loading"><span/><span/><span/></div>:movements.length===0?<div className="stock-movement-empty"><i className="bi bi-clock-history"/><p>No stock movements yet.</p></div>:movements.map((movement)=><article key={movement.id}><span className={`movement-icon is-${movement.type}`}><i className={`bi ${movement.type==='order_placed'?'bi-cart-check':'bi-sliders'}`}/></span><div><header><strong>{movement.type.replaceAll('_',' ')}</strong><time>{formatDate(movement.created_at)}</time></header><p>{movement.reason||'No reason provided.'}</p><div><span>Before <b>{movement.quantity_before}</b></span><span className={movement.quantity_changed>=0?'positive':'negative'}>{movement.quantity_changed>=0?'+':''}{movement.quantity_changed}</span><span>After <b>{movement.quantity_after}</b></span></div>{movement.reference&&<small>Reference: {movement.reference.type} #{movement.reference.id}</small>}{movement.created_by&&<small>By {movement.created_by.name}</small>}</div></article>)}</div>{movementMeta?.last_page>1&&<footer className="stock-history-pagination"><button type="button" disabled={movementPage<=1} onClick={()=>{setMovementLoading(true);setMovementPage(p=>p-1)}}>Previous</button><span>{movementMeta.current_page} / {movementMeta.last_page}</span><button type="button" disabled={movementPage>=movementMeta.last_page} onClick={()=>{setMovementLoading(true);setMovementPage(p=>p+1)}}>Next</button></footer>}</section></div>}
  </main>
}
